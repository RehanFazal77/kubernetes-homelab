...
echo "=== Kubernetes Control Plane Setup Started ==="
...
# 2. Disable Swap, if swap reamins turned on then it affects the k8s scheduler
...
echo "=== 11. Deploying Flannel CNI ==="
retry kubectl apply -f https://raw.githubusercontent.com/flannel-io/flannel/master/Documentation/kube-flannel.yml

# 12. Wait for Flannel pods to be Running
echo "=== 12. Waiting for Flannel pods to be Running ==="
until kubectl get pods -n kube-flannel | grep -v Running | grep -q .; do
